...
bool String::operator < (const String& other) const{ //por orden alfab�tico  a < z
        int comp = strcmp(cadena, other.ToCString());
        if (comp < 0)
                return true;
        else
...
String String::Replace(const String& find, const String& rep) const{    
        ...
        while (aparicion_now <= apariciones) 
        {
                ...
                if (aparicion_now < apariciones) {
                        // Mientras la posici�n actual sea menor que el final de cadena
                        while(pos <= Length() + 1)
                        {
                ...
                aparicion_index = pos; // Actualizamos el �ndice de la palabra a despues de la palabra encontrada
